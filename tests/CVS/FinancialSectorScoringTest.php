...
class FinancialSectorScoringTest extends TestCase
{
    ...
    public function testCheaperBookMultipleScoresHigher(): void
    {
        $cheap = $this->pillar()->score($this->bank(1.38)); // ALR.WA
        $rich  = $this->pillar()->score($this->bank(2.96)); // ING.WA
    ...
    {
        $this->repo->upsertMedian('industry', 'Banks - Regional', 'Financial Services', (string) $this->config['model_version'], 'pb', 2.30, 8);

        $pillar = $this->pillar();
        $pillar->score($this->bank(2.32));

        $this->assertSame('subsector', $pillar->lastSource());
        $this->assertSame(2.30, $pillar->steps()['sub_median']);
    }
