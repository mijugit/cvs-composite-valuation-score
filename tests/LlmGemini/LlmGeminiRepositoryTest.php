 ...
 */
class LlmGeminiRepositoryTest extends TestCase
{
    private PDO                $db;
    private LlmGeminiRepository $repo;
    ...
    protected function setUp(): void
    {
        ...
        $this->db->exec('
            CREATE TABLE llm_gemini_holdings (
                id              INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        $this->repo = new LlmGeminiRepository($this->db);
    }
    ...
    {
        $this->db->exec("INSERT INTO llm_gemini_state (cash, initial_capital, updated_at)
                         VALUES (10000.00, 10000.00, '2026-08-19 12:00:00')");
    ...
    {
        $this->db->exec("INSERT INTO llm_gemini_holdings (ticker, quantity, avg_entry_price, updated_at)
                         VALUES ('AAPL', 10, 150.0000, '2026-08-19 12:00:00')");
        $this->db->exec("INSERT INTO llm_gemini_holdings (ticker, quantity, avg_entry_price, updated_at)
                         VALUES ('MSFT', 0, 300.0000, '2026-08-19 12:00:00')");
    ...
    {
        $this->db->exec("INSERT INTO llm_gemini_holdings (ticker, quantity, avg_entry_price, updated_at)
                         VALUES ('NVDA', 5, 800.0, '2026-08-19 12:00:00')");
        $this->db->exec("INSERT INTO llm_gemini_holdings (ticker, quantity, avg_entry_price, updated_at)
                         VALUES ('AAPL', 10, 150.0, '2026-08-19 12:00:00')");
    ...
    {
        $this->db->exec("INSERT INTO llm_gemini_holdings (ticker, quantity, avg_entry_price, updated_at)
                         VALUES ('AVGO', 10, 180.00, '2026-08-15 12:00:00')");
        $this->db->exec("INSERT INTO cvs_snapshots
                         ...
                         VALUES ('AVGO', 'Broadcom Inc.', '4.0', 'RESCORE', '⬆ AKUMULUJ', '⬆ AKUMULUJ',
                          57.9, 54.3, 208.50, '2026-08-16', '2026-08-16 16:00:00')");
    ...
    {
        $this->db->exec("INSERT INTO llm_gemini_holdings (ticker, quantity, avg_entry_price, updated_at)
                         VALUES ('NEWCO', 5, 50.00, '2026-08-15 12:00:00')");
    ...
    {
        $this->db->exec("INSERT INTO llm_gemini_cycle (cycle_date, status, started_at)
                         VALUES ('2026-08-17', 'completed', '2026-08-17 21:40:00')");
        $this->db->exec("INSERT INTO llm_gemini_cycle (cycle_date, status, started_at)
        ...
        $this->assertSame('2026-08-19', $latest['cycle_date']);
    }
    ...
    {
        $this->db->exec("INSERT INTO llm_gemini_cycle (cycle_date, status, started_at, legend)
                         VALUES ('2026-08-17', 'failed', '2026-08-17 21:40:00', NULL)");
        $this->db->exec("INSERT INTO llm_gemini_cycle (cycle_date, status, started_at, legend)
                         VALUES ('2026-08-18', 'completed', '2026-08-18 21:40:00', 'Kupiłem AVGO — marże rosną.')");

        $history = $this->repo->getLegendHistory(10);

        $this->assertCount(1, $history);
        $this->assertSame('2026-08-18', $history[0]['cycle_date']);
    ...
    public function testGetLegendHistoryReturnsNewestFirstLimitedToN(): void
    {
        for ($i = 1; $i <= 15; $i++) {
            ...
            $this->db->exec("INSERT INTO llm_gemini_cycle (cycle_date, status, started_at, legend)
                             VALUES ('$date', 'completed', '$date 21:40:00', 'Wpis $i')");
        }
