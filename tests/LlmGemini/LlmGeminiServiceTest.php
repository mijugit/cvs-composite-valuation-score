...
class LlmGeminiServiceTest extends TestCase
{
    private PDO                      $db;
    private LlmGeminiCycleRepository $cycleRepo;
    ...
    protected function setUp(): void
    {
        ...
        $this->db->exec('
            CREATE TABLE llm_gemini_transactions (
                id          INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        // Seed wallet state with 10 000 USD — same starting capital as the other two wallets.
        $this->db->exec("INSERT INTO llm_gemini_state (cash, initial_capital, updated_at)
                         VALUES (10000.00, 10000.00, '2026-08-19 21:40:00')");

        $this->cycleRepo = new LlmGeminiCycleRepository($this->db);
        $this->service   = new LlmGeminiService($this->db, $this->cycleRepo);
    }
    ...
    {
        $row = $this->db->query('SELECT * FROM llm_gemini_state LIMIT 1')->fetch(PDO::FETCH_ASSOC);
    ...
    {
        return $this->db->query('SELECT * FROM llm_gemini_holdings ORDER BY ticker')->fetchAll(PDO::FETCH_ASSOC);
    }

    /** @return array<int, array<string, mixed>> */
    private function fetchTransactions(int $cycleId): array
    {
        $stmt = $this->db->prepare('SELECT * FROM llm_gemini_transactions WHERE cycle_id = ? ORDER BY id');
    ...
    {
        $stmt = $this->db->prepare('SELECT * FROM llm_gemini_cycle WHERE id = ?');
        $stmt->execute([$id]);
    ...
    {
        $this->db->exec('DELETE FROM llm_gemini_state');
    ...
    {
        $this->db->exec("UPDATE llm_gemini_state SET cash = 100.00");
    ...
    public function testExceptionMidCycleRollsBackAllChanges(): void
    {
        ...
        $this->db->exec('DROP TABLE llm_gemini_transactions');
