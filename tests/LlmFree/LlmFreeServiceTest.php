...
class LlmFreeServiceTest extends TestCase
{
    private PDO                    $db;
    private LlmFreeCycleRepository $cycleRepo;
    private LlmFreeService         $service;
    ...
    protected function setUp(): void
    {
        ...
        $this->db->exec('
            CREATE TABLE llm_free_transactions (
                id          INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        // Seed wallet state with 10 000 USD — same starting capital as the baseline wallet.
        $this->db->exec("INSERT INTO llm_free_state (cash, initial_capital, updated_at)
                         VALUES (10000.00, 10000.00, '2026-08-07 21:30:00')");

        $this->cycleRepo = new LlmFreeCycleRepository($this->db);
        $this->service   = new LlmFreeService($this->db, $this->cycleRepo);
    }
    ...
    {
        $row = $this->db->query('SELECT * FROM llm_free_state LIMIT 1')->fetch(PDO::FETCH_ASSOC);
    ...
    {
        return $this->db->query('SELECT * FROM llm_free_holdings ORDER BY ticker')->fetchAll(PDO::FETCH_ASSOC);
    }

    /** @return array<int, array<string, mixed>> */
    private function fetchTransactions(int $cycleId): array
    {
        $stmt = $this->db->prepare('SELECT * FROM llm_free_transactions WHERE cycle_id = ? ORDER BY id');
    ...
    {
        $stmt = $this->db->prepare('SELECT * FROM llm_free_cycle WHERE id = ?');
        $stmt->execute([$id]);
    ...
    {
        $this->db->exec('DELETE FROM llm_free_state');
    ...
    {
        $this->db->exec("UPDATE llm_free_state SET cash = 100.00");
    ...
    {
        // 60 shares at $100 = $6000 → 60% of the $10000 base, well past the
        // baseline wallet's 15% single-position / 40% sector caps.
        $id = $this->insertCycle();
        $this->service->executeCycle($id, [[
    ...
    public function testSellIncreasesCashAndRemovesHolding(): void
    {
        $this->db->exec("INSERT INTO llm_free_holdings (ticker, quantity, avg_entry_price, updated_at)
                         VALUES ('AAPL', 10, 150.0000, '2026-08-07 21:30:00')");
    ...
    {
        // Only 5 shares held — a SELL requesting 20 must cap at 5, not oversell.
        $this->db->exec("INSERT INTO llm_free_holdings (ticker, quantity, avg_entry_price, updated_at)
                         VALUES ('AAPL', 5, 150.0000, '2026-08-07 21:30:00')");
    ...
    public function testTwoBuySameTickerWeightedAverage(): void
    {
        ...
        $this->db->exec("INSERT INTO llm_free_cycle (cycle_date, status, started_at)
                         VALUES ('2026-08-08', 'started', '2026-08-08 21:30:00')");
    ...
    public function testExceptionMidCycleRollsBackAllChanges(): void
    {
        ...
        $this->db->exec('DROP TABLE llm_free_transactions');
