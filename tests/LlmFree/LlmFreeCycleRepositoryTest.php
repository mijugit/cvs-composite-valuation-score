...
class LlmFreeCycleRepositoryTest extends TestCase
{
    private PDO                   $db;
    private LlmFreeCycleRepository $repo;
    ...
    protected function setUp(): void
    {
        ...
        $this->repo = new LlmFreeCycleRepository($this->db);
    }

    /** @return array<string,mixed> */
    private function fetch(string $date): array
    {
        $stmt = $this->db->prepare('SELECT * FROM llm_free_cycle WHERE cycle_date = ?');
        $stmt->execute([$date]);
    ...
    {
        $id = $this->repo->claimForRun('2026-08-07', 3);

        $this->assertNotNull($id);
        $row = $this->fetch('2026-08-07');
    ...
    {
        $this->db->exec("INSERT INTO llm_free_cycle (cycle_date, status, attempt_count) VALUES ('2026-08-07', 'completed', 1)");

        $this->assertNull($this->repo->claimForRun('2026-08-07', 3));
    }

    public function testStartedCycleIsNotReRun(): void
    {
        $this->db->exec("INSERT INTO llm_free_cycle (cycle_date, status, attempt_count) VALUES ('2026-08-07', 'started', 1)");
    ...
    {
        $this->db->exec("INSERT INTO llm_free_cycle (cycle_date, status, attempt_count, finished_at) VALUES ('2026-08-07', 'llm_failed', 1, CURRENT_TIMESTAMP)");
    ...
    {
        $this->db->exec("INSERT INTO llm_free_cycle (cycle_date, status, attempt_count) VALUES ('2026-08-07', 'failed', 3)");

        $this->assertNull($this->repo->claimForRun('2026-08-07', 3));
        $this->assertSame(3, (int) $this->fetch('2026-08-07')['attempt_count']);
    }

    public function testRetryAllowedAtSecondAttemptWithMaxThree(): void
    {
        $this->db->exec("INSERT INTO llm_free_cycle (cycle_date, status, attempt_count) VALUES ('2026-08-07', 'failed', 2)");
