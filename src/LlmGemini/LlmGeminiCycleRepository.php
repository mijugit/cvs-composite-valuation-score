...
/**
 * Data-access layer for the llm_gemini_cycle table.
 *
 * Structural clone of CVS\LlmFree\LlmFreeCycleRepository (change:
 * llm-gemini-wallet) — identical claim/status/summary contract, table renamed.
 */
class LlmGeminiCycleRepository
{
    ...
    public function findTodayCycle(string $cycleDate): ?array
    {
        $stmt = $this->db->prepare(
            'SELECT * FROM llm_gemini_cycle WHERE cycle_date = ? LIMIT 1'
        );
     ...
     * comparison chart on /llm-gemini. Mirrors
     * CVS\LlmFree\LlmFreeCycleRepository::getValueSeries() exactly, one table over.
     *
    ...
    public function claimForRun(string $cycleDate, int $maxAttempts): ?int
    {
        ...
        $stmt = $this->db->prepare(
            'INSERT INTO llm_gemini_cycle (cycle_date, status, attempt_count, started_at)
             SELECT ?, \'started\', 1, CURRENT_TIMESTAMP
             WHERE NOT EXISTS (SELECT 1 FROM llm_gemini_cycle WHERE cycle_date = ?)'
        );
    ...
    {
        $stmt = $this->db->prepare(
            'UPDATE llm_gemini_cycle SET status = ?, finished_at = CURRENT_TIMESTAMP WHERE id = ?'
        );
     ...
     * Called inside the open DB transaction in LlmGeminiService::executeCycle().
     */
