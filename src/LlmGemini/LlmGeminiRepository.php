...
/**
 * Read-only access to all LLM_Gemini_Wallet tables.
 *
 * Pure SELECT methods — no side effects, no transactions. Structural clone of
 * CVS\LlmFree\LlmFreeRepository (change: llm-gemini-wallet) — same method
 * surface, tables renamed llm_gemini_*.
 */
class LlmGeminiRepository
{
    public function __construct(private readonly PDO $db) {}

    /**
     * Returns the single llm_gemini_state row.
     *
     * @return array<string, mixed>
     * @throws \RuntimeException when the wallet has not been initialized (no row in llm_gemini_state)
     */
    public function getCurrentState(): array
    {
        $stmt = $this->db->prepare('SELECT * FROM llm_gemini_state LIMIT 1');
        $stmt->execute();
        $row = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($row === false) {
            throw new \RuntimeException('LLM_Gemini_Wallet not initialized: llm_gemini_state is empty.');
        }
    ...
    /**
     * Returns the most recent llm_gemini_cycle row, or null if none exists.
     *
    ...
    {
        $stmt = $this->db->prepare(
            'SELECT * FROM llm_gemini_cycle ORDER BY cycle_date DESC LIMIT 1'
        );
        $stmt->execute();
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
