...
/**
 * Atomic write model for the LLM_Gemini_Wallet.
 *
 * Structural clone of CVS\LlmFree\LlmFreeService (change: llm-gemini-wallet) —
 * identical transaction-per-cycle shape, identical physical guards (a BUY that
 * costs more than available cash is skipped, a SELL is capped at the quantity
 * actually held), no DecisionEnforcer-equivalent, mark-to-market from the first
 * line of code. Tables renamed llm_gemini_*.
 *
 * The decision array consumed by executeCycle() is produced by
 * LlmGeminiDecisionService. Each element must have: ticker (string), action
...
class LlmGeminiService
{
     ...
     *
     * @throws \RuntimeException when llm_gemini_state does not contain exactly one row
     */
    public function ensureInitialized(): void
    {
        $stmt = $this->db->prepare('SELECT COUNT(*) FROM llm_gemini_state');
        $stmt->execute();
        $count = (int) $stmt->fetchColumn();

        if ($count !== 1) {
            throw new \RuntimeException(
                'LLM_Gemini_Wallet not initialized: expected 1 row in llm_gemini_state, found ' . $count . '.'
            );
        }
    }
     ...
     *
     * All writes (llm_gemini_state, llm_gemini_holdings, llm_gemini_transactions,
     * llm_gemini_cycle summary) happen in one PDO transaction. Any Throwable
     ...
     *        on /llm-gemini — used to surface decisions the caller had to discard
    ...
    public function executeCycle(int $cycleId, array $decisions, array $priceMap, ?string $notes = null): void
    {
        ...
        try {
            ...
            $stmt = $this->db->prepare('SELECT cash FROM llm_gemini_state LIMIT 1');
            $stmt->execute();
    ...
    private function executeBuyInternal(
        ...
        // Reduce cash.
        $this->db->prepare('UPDATE llm_gemini_state SET cash = ?, updated_at = CURRENT_TIMESTAMP')
        ...
        if ($existing !== null) {
            ...
            $this->db->prepare(
                'UPDATE llm_gemini_holdings SET quantity = ?, avg_entry_price = ?, updated_at = CURRENT_TIMESTAMP WHERE ticker = ?'
            )->execute([$newQty, $newAvg, $ticker]);
        } else {
            $this->db->prepare(
                'INSERT INTO llm_gemini_holdings (ticker, quantity, avg_entry_price, updated_at)
    ...
    private function executeSellInternal(
        ...
        if ($existing !== null) {
            ...
            if ($remaining <= 0) {
                $this->db->prepare('DELETE FROM llm_gemini_holdings WHERE ticker = ?')
                    ->execute([$ticker]);
            } else {
                $this->db->prepare(
                    'UPDATE llm_gemini_holdings SET quantity = ?, updated_at = CURRENT_TIMESTAMP WHERE ticker = ?'
    ...
    {
        $stmt = $this->db->prepare('SELECT * FROM llm_gemini_holdings WHERE ticker = ? LIMIT 1');
        $stmt->execute([$ticker]);
    ...
    private function insertTransaction(
        ...
        $this->db->prepare(
            'INSERT INTO llm_gemini_transactions
     ...
     * code (change: llm-gemini-wallet, same discipline as the sibling
     * LLM_Free_Wallet module).
     *
     * @param array<string, float> $priceMap ticker (uppercase) => today's USD snapshot price
     */
    private function computeHoldingsValue(array $priceMap): float
    {
        $stmt = $this->db->prepare(
            'SELECT ticker, quantity, avg_entry_price FROM llm_gemini_holdings WHERE quantity > 0'
        );
        $stmt->execute();
