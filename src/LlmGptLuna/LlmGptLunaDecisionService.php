...
/**
 * LLM decision pipeline for the GPT-Luna wallet — structural clone of
 * CVS\LlmGemini\LlmGeminiDecisionService (change: llm-gpt-luna-wallet), with
 * GPTClient/GPTClientFactory swapped in for GeminiClient/GeminiClientFactory.
 *
 ...
 * entirely provider-agnostic (deliberate reuse, not a duplicate).
 *
 * The caller (bin/llm-gpt-luna-wallet-rebalance.php) acts on the returned result:
 *   ok=true  → pass $decisions to LlmGptLunaService::executeCycle()
 *   ok=false → mark cycle llm_failed, exit without touching the wallet
 */
class LlmGptLunaDecisionService
{
    /**
     * @param array<string, mixed> $gptConfig merged config/gpt.php + llm-gpt-luna-wallet.php['llm']
     * @param array<string, mixed> $walletConfig config/llm-gpt-luna-wallet.php
     */
     ...
     *
     * @param array<string, mixed>              $portfolioState  from LlmGptLunaRepository::getCurrentState()
     * @param array<int, array<string, mixed>>  $holdings        from LlmGptLunaRepository::getCurrentHoldings()
     * @param array<int, array<string, mixed>>  $screenerRows    candidate universe (CVS snapshot rows)
     * @param array<int, array{cycle_date: string, legend: string}> $legendHistory from LlmGptLunaRepository::getLegendHistory()
     * @param array<string, string>             $contextByTicker from LlmGptLunaContextGatherer::gather()
     *
