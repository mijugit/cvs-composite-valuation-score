...
/**
 * Atomic write model for the LLM_GPT_Luna_Wallet.
 *
 * Structural clone of CVS\LlmGemini\LlmGeminiService (change:
 * llm-gpt-luna-wallet) — identical transaction-per-cycle shape, identical
 * physical guards (a BUY that costs more than available cash is skipped, a
 * SELL is capped at the quantity actually held), no DecisionEnforcer-
 * equivalent, mark-to-market from the first line of code. Tables renamed
 * llm_gpt_luna_*.
 *
 * The decision array consumed by executeCycle() is produced by
 * LlmGptLunaDecisionService. Each element must have: ticker (string), action
 * (string), quantity (?int), price_usd (?float), reason (?string).
 */
class LlmGptLunaService
{
    public function __construct(
        private readonly PDO                       $db,
        private readonly LlmGptLunaCycleRepository $cycleRepo,
    ) {}

    /**
     * Guards against an uninitialized wallet (missing seed row).
     *
     * @throws \RuntimeException when llm_gpt_luna_state does not contain exactly one row
     */
    public function ensureInitialized(): void
    {
        $stmt = $this->db->prepare('SELECT COUNT(*) FROM llm_gpt_luna_state');
        $stmt->execute();
        $count = (int) $stmt->fetchColumn();

        if ($count !== 1) {
            throw new \RuntimeException(
                'LLM_GPT_Luna_Wallet not initialized: expected 1 row in llm_gpt_luna_state, found ' . $count . '.'
            );
        }
    }
     ...
     *
     * All writes (llm_gpt_luna_state, llm_gpt_luna_holdings,
     * llm_gpt_luna_transactions, llm_gpt_luna_cycle summary) happen in one
     * PDO transaction. Any Throwable triggers a full rollback; the exception
     ...
     *        Marks the cycle-end valuation to market, never cost basis.
     * @param string|null $notes Operational note stored on the cycle row and shown
     *        on /llm-gpt-luna — used to surface decisions the caller had to discard
     *        (e.g. a BUY/SELL with no known price).
     */
    public function executeCycle(int $cycleId, array $decisions, array $priceMap, ?string $notes = null): void
    {
        ...
        try {
            ...
            $stmt = $this->db->prepare('SELECT cash FROM llm_gpt_luna_state LIMIT 1');
            $stmt->execute();
    ...
    private function executeBuyInternal(
        ...
        // Reduce cash.
        $this->db->prepare('UPDATE llm_gpt_luna_state SET cash = ?, updated_at = CURRENT_TIMESTAMP')
        ...
        if ($existing !== null) {
            ...
            $this->db->prepare(
                'UPDATE llm_gpt_luna_holdings SET quantity = ?, avg_entry_price = ?, updated_at = CURRENT_TIMESTAMP WHERE ticker = ?'
            )->execute([$newQty, $newAvg, $ticker]);
        } else {
            $this->db->prepare(
                'INSERT INTO llm_gpt_luna_holdings (ticker, quantity, avg_entry_price, updated_at)
    ...
    private function executeSellInternal(
        ...
        if ($existing !== null) {
            ...
            if ($remaining <= 0) {
                $this->db->prepare('DELETE FROM llm_gpt_luna_holdings WHERE ticker = ?')
                    ->execute([$ticker]);
            } else {
                $this->db->prepare(
                    'UPDATE llm_gpt_luna_holdings SET quantity = ?, updated_at = CURRENT_TIMESTAMP WHERE ticker = ?'
    ...
    {
        $stmt = $this->db->prepare('SELECT * FROM llm_gpt_luna_holdings WHERE ticker = ? LIMIT 1');
        $stmt->execute([$ticker]);
    ...
    private function insertTransaction(
        ...
        $this->db->prepare(
            'INSERT INTO llm_gpt_luna_transactions
     ...
     * code (change: llm-gpt-luna-wallet, same discipline as both sibling
     * modules).
     *
     * @param array<string, float> $priceMap ticker (uppercase) => today's USD snapshot price
     */
    private function computeHoldingsValue(array $priceMap): float
    {
        $stmt = $this->db->prepare(
            'SELECT ticker, quantity, avg_entry_price FROM llm_gpt_luna_holdings WHERE quantity > 0'
        );
        $stmt->execute();
