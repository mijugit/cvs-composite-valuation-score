...
/**
 * Data-access layer for the llm_gpt_luna_cycle table.
 *
 * Structural clone of CVS\LlmGemini\LlmGeminiCycleRepository (change:
 * llm-gpt-luna-wallet) — identical claim/status/summary contract, table renamed.
 */
class LlmGptLunaCycleRepository
{
    ...
    public function findTodayCycle(string $cycleDate): ?array
    {
        $stmt = $this->db->prepare(
            'SELECT * FROM llm_gpt_luna_cycle WHERE cycle_date = ? LIMIT 1'
        );
     ...
     * comparison chart on /llm-gpt-luna. Mirrors
     * CVS\LlmGemini\LlmGeminiCycleRepository::getValueSeries() exactly, one table over.
     *
    ...
    public function claimForRun(string $cycleDate, int $maxAttempts): ?int
    {
        ...
        $stmt = $this->db->prepare(
            'INSERT INTO llm_gpt_luna_cycle (cycle_date, status, attempt_count, started_at)
             SELECT ?, \'started\', 1, CURRENT_TIMESTAMP
             WHERE NOT EXISTS (SELECT 1 FROM llm_gpt_luna_cycle WHERE cycle_date = ?)'
        );
    ...
    {
        $stmt = $this->db->prepare(
            'UPDATE llm_gpt_luna_cycle SET status = ?, finished_at = CURRENT_TIMESTAMP WHERE id = ?'
        );
     ...
     * model's legend entry and token usage. Called OUTSIDE the wallet's
     * execution DB transaction (audit must persist even on rollback).
     */
     ...
     * Called inside the open DB transaction in LlmGptLunaService::executeCycle().
     */
