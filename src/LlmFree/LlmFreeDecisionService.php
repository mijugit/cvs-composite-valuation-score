...
/**
 * LLM decision pipeline for the free-form wallet.
 *
 * Builds a free-form prompt (full interpretive discretion over CVS signals,
 * no fixed thresholds, no obligation to act in the wallet's interest — PRD
 * FR-002/FR-004) plus the model's own legend memory, calls ClaudeClient with
 * exactly 0 internal retries (service-level retry owns the 2-attempt
 * contract, mirroring CVS\Portfolio\DecisionService), parses the combined
 * {decisions, legend} response through LlmFreeDecisionParser, and writes the
 * LLM audit record (including legend + token usage) to llm_free_cycle.
 *
 * The caller (bin/llm-free-wallet-rebalance.php) acts on the returned result:
 *   ok=true  → pass $decisions to LlmFreeService::executeCycle()
 *   ok=false → mark cycle llm_failed, exit without touching the wallet
 */
class LlmFreeDecisionService
{
    /**
     * @param array<string, mixed> $aiConfig     merged ai.php + llm-free-wallet.php['llm']
     * @param array<string, mixed> $walletConfig llm-free-wallet.php
     */
     ...
     *
     * @param array<string, mixed>              $portfolioState  from LlmFreeRepository::getCurrentState()
     * @param array<int, array<string, mixed>>  $holdings        from LlmFreeRepository::getCurrentHoldings()
     * @param array<int, array<string, mixed>>  $screenerRows    candidate universe (CVS snapshot rows)
     * @param array<int, array{cycle_date: string, legend: string}> $legendHistory from LlmFreeRepository::getLegendHistory()
     * @param array<string, string>             $contextByTicker from LlmFreeContextGatherer::gather()
     *
