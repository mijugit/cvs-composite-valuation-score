...
/**
 * Read-only access to all LLM_Free_Wallet tables.
 *
 * Pure SELECT methods — no side effects, no transactions. Mirrors
 * CVS\Portfolio\PortfolioRepository's method surface for the sibling wallet.
 */
class LlmFreeRepository
{
    public function __construct(private readonly PDO $db) {}

    /**
     * Returns the single llm_free_state row.
     *
     * @return array<string, mixed>
     * @throws \RuntimeException when the wallet has not been initialized (no row in llm_free_state)
     */
    public function getCurrentState(): array
    {
        $stmt = $this->db->prepare('SELECT * FROM llm_free_state LIMIT 1');
        $stmt->execute();
        $row = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($row === false) {
            throw new \RuntimeException('LLM_Free_Wallet not initialized: llm_free_state is empty.');
        }
    ...
    /**
     * Returns the most recent llm_free_cycle row, or null if none exists.
     *
    ...
    {
        $stmt = $this->db->prepare(
            'SELECT * FROM llm_free_cycle ORDER BY cycle_date DESC LIMIT 1'
        );
        $stmt->execute();
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
