...
- **S-05 dual-mode architecture (3 pillars):**
  - `ValuationPillar` — three variants, chosen per company, all scored the same way (ratio vs a resolved peer median → sigmoid → sector anchor):
    ...
    - **C** — price/book CONDITIONED ON ROE (`P/B ÷ ROE` vs the peer median of the same), for sectors in `config → financials.sectors`. A bank's EV and free cash flow measure nothing, and its book multiple is a function of the return that earns it — Gordon-Shapiro, `P/B = (ROE - g)/(COE - g)`. Comparing raw P/B to a median scored profitability as expense: the measured ROE-vs-score correlation was **-0.54** before this, +0.27 after. Beyond `financials.pb_roe.implausible_multiple` of the peer median the pillar returns neutral rather than a confident extreme — that range is where ADR unit errors land (HSBC reads 4.5x because Yahoo divides an ADR price by an ordinary-share book value, and Yahoo returns no balance sheet for banks to rebuild it from). **Fair value must use the same axis** (`median_pb_roe × ROE × BVPS`) or it contradicts the pillar beside it.
