...
/**
 * Background worker for the GPT-5.6 Terra side of the stage-2 "Recenzja
 ...
 * 'terra') instead — a deliberately separate script per flavor so each
 ...
 *
 * Contract: php bin/generate_critical_review_gpt_terra.php <ticker> <userId>
 *
...
$logFile = ROOT_PATH . '/logs/critical_review_gpt_terra.log';
...
if ($ticker === '' || $userId <= 0) {
    $log(sprintf('generate_critical_review_gpt_terra: ERROR invalid args — ticker=%s userId=%s', $argv[1] ?? '', $argv[2] ?? ''));
    exit(1);
}

$ticker = strtoupper($ticker);

$log("generate_critical_review_gpt_terra: start ticker={$ticker} userId={$userId}");
...
try {
    ...
    if ($stage1 === null || !isset($stage1['content'])) {
        $reviewRepo->markFailed($ticker, CriticalReviewProvider::GPT_TERRA, 'Brak analizy etapu 1 dla tej spółki.');
        $log("generate_critical_review_gpt_terra: FAILED {$ticker} — no stage-1 analysis");
        exit(0);
    }
    ...
    if ($financials === null) {
        $reviewRepo->markFailed($ticker, CriticalReviewProvider::GPT_TERRA, 'Nie udało się pobrać danych rynkowych.');
        $log("generate_critical_review_gpt_terra: FAILED {$ticker} — fetch failed");
        exit(0);
    }
    ...
    $service = new GPTCriticalReviewService(
        GPTClientFactory::fromConfig($gptConfig, 'terra'),
    ...
    if (!$result->ok) {
        $reviewRepo->markFailed($ticker, CriticalReviewProvider::GPT_TERRA, (string) ($result->failureMessage ?? 'Nieznany błąd generowania.'));
        $log(sprintf('generate_critical_review_gpt_terra: FAILED %s — %s', $ticker, $result->failureMessage ?? 'unknown'));
        exit(0);
    }
    ...
    $log(sprintf(
        'generate_critical_review_gpt_terra: done ticker=%s tokens_in=%d tokens_out=%d search_degraded=%s probability_parsed=%s',
        $ticker,
        $tokensIn,
        $tokensOut,
    ...
    $reviewRepo->markFailed($ticker, CriticalReviewProvider::GPT_TERRA, 'Błąd wewnętrzny generowania recenzji.');
    $log(sprintf('generate_critical_review_gpt_terra: FATAL %s — %s in %s:%d', $ticker, $e->getMessage(), $e->getFile(), $e->getLine()));
    exit(1);
}
