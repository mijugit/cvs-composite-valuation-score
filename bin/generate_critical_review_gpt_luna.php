...
/**
 * Background worker for the GPT-5.6 Luna side of the stage-2 "Recenzja
 * krytyczna" feature — change: critical-review-openai.
 *
 * Mirrors bin/generate_critical_review_gemini.php's structure (CLI guard,
 * .env parsing, $_SESSION reset, stage-1 + CVS re-enrichment, try/catch
 * envelope), wired to GPTCriticalReviewService/GPTClientFactory (flavor
 * 'luna') instead — a deliberately separate script per flavor so each
 * provider's async job stays independent, even though Terra and Luna share
 * the identical GPTClient/GPTCriticalReviewService implementation (see
 * GPTClientFactory's docblock — the two flavors differ only in model ID and
 * API key, not in transport/request shape).
 *
 * No HTTP session access here (background CLI process) — ticker and userId
 * arrive as positional CLI arguments, not from $_SESSION. Provider/flavor is
 * implicit in which script runs — not a third CLI arg — so a caller can
 * never pass a mismatched provider value.
 *
 * Contract: php bin/generate_critical_review_gpt_luna.php <ticker> <userId>
 *
...
$logFile = ROOT_PATH . '/logs/critical_review_gpt_luna.log';
...
if ($ticker === '' || $userId <= 0) {
    $log(sprintf('generate_critical_review_gpt_luna: ERROR invalid args — ticker=%s userId=%s', $argv[1] ?? '', $argv[2] ?? ''));
    exit(1);
}

$ticker = strtoupper($ticker);

$log("generate_critical_review_gpt_luna: start ticker={$ticker} userId={$userId}");
...
try {
    ...
    if ($stage1 === null || !isset($stage1['content'])) {
        $reviewRepo->markFailed($ticker, CriticalReviewProvider::GPT_LUNA, 'Brak analizy etapu 1 dla tej spółki.');
        $log("generate_critical_review_gpt_luna: FAILED {$ticker} — no stage-1 analysis");
        exit(0);
    }
    ...
    if ($financials === null) {
        $reviewRepo->markFailed($ticker, CriticalReviewProvider::GPT_LUNA, 'Nie udało się pobrać danych rynkowych.');
        $log("generate_critical_review_gpt_luna: FAILED {$ticker} — fetch failed");
        exit(0);
    }
    ...
    // for network purposes) Claude client, same as the Gemini worker.
    $service = new GPTCriticalReviewService(
        GPTClientFactory::fromConfig($gptConfig, 'luna'),
    ...
    if (!$result->ok) {
        $reviewRepo->markFailed($ticker, CriticalReviewProvider::GPT_LUNA, (string) ($result->failureMessage ?? 'Nieznany błąd generowania.'));
        $log(sprintf('generate_critical_review_gpt_luna: FAILED %s — %s', $ticker, $result->failureMessage ?? 'unknown'));
        exit(0);
    }
    ...
    $reviewRepo->markCompleted(
        $ticker,
        CriticalReviewProvider::GPT_LUNA,
        $parsed['narrative'],
        $result->citations,
        (string) ($result->model ?? ''),
        $tokensIn,
    ...
    $log(sprintf(
        'generate_critical_review_gpt_luna: done ticker=%s tokens_in=%d tokens_out=%d search_degraded=%s probability_parsed=%s',
        $ticker,
        $tokensIn,
        $tokensOut,
    ...
    $reviewRepo->markFailed($ticker, CriticalReviewProvider::GPT_LUNA, 'Błąd wewnętrzny generowania recenzji.');
    $log(sprintf('generate_critical_review_gpt_luna: FATAL %s — %s in %s:%d', $ticker, $e->getMessage(), $e->getFile(), $e->getLine()));
    exit(1);
}
